...
(function () {
    ...
    PHASES.forEach(function (phase) {
      ...
      phase.sections.forEach(function (sId) {
        ...
        if (s.gammaDecks && s.gammaDecks.length > 0) {
          html += '<span class="gamma-badge">Gamma Deck</span>';
        }
        if (score !== undefined) {
          var passed = score >= 3;
  ...
  function renderChapterContent(section, chapterIdx) {
    ...
    html += '<div class="chapter-header"><h1>' + ch.title + '</h1>';

    // Gamma decks on first chapter
    if (isFirst && section.gammaDecks && section.gammaDecks.length > 0) {
      html += '<div class="gamma-links">';
      section.gammaDecks.forEach(function (deck) {
        html += '<a href="' + deck.url + '" target="_blank" rel="noopener noreferrer" class="gamma-link">' +
          ICONS.external + ' ' + deck.title + '</a>';
      });
      html += '</div>';
    }
